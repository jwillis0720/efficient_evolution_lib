...
import sys
import torch
from efficient_evolution.fb_model import FBModel
...
def decode(embedding, model, exclude=set()):
    ...
    valid_idx = [idx for idx, tok in enumerate(model.alphabet_.all_toks)]
    ...
    argmax = "".join(
        [
            ...
            if (
                "<" not in model.alphabet_.all_toks[valid_idx[tok_idx]]
                and model.alphabet_.all_toks[valid_idx[tok_idx]] not in exclude
            )
            else "."
...
def reconstruct(seq, model, encode_kwargs={}, decode_kwargs={}):
    ...
    return decode(encode(seq, model, **encode_kwargs), model, **decode_kwargs)
...
def reconstruct_prose(seq, model):
    ...
    return "".join([alphabet.decode(np.argmax(logits[i])).decode("utf-8") for i in range(logits.shape[0])])
...
def compare(seq_old, seq_new, start=0, end=None, namespace=None):
    ...
    for idx, (ch_old, ch_new) in enumerate(zip(seq_old, seq_new)):
        if idx < start:
            continue
...
def interpolate(baseline, target, model, n_steps=15, start=0, end=None):
    for alpha in np.linspace(0.0, 1.0, n_steps):
        midpoint = (alpha * encode(baseline, model)) + ((1 - alpha) * encode(target, model))
        new_seq = decode(midpoint, model)
        compare(target, new_seq, start, end)


def extrapolate(baseline, target, model, n_steps=15, start=0, end=None):
    for alpha in np.linspace(1.0, 5.0, n_steps):
